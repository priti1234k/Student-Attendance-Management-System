...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($facname_err) && empty($email_err) && empty($contact_err) && empty($gender_err) && empty($username_err) && empty($password_err)){
        ...
        $sql = "UPDATE faculty_master SET facultyname=? , Email=? , Contact=?, gender=?, Username=?, Password=? WHERE Fac_id=?";       
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM faculty_master WHERE Fac_id = ?";
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
...
<body>
	...
	<h3>Update Account</h3>
	 <p>Please edit the input values and submit to update the Faculty record.</p>

	
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="POST">
                        <div class="form-group">
                            <label>Faculty Name</label>
                            <input type="text" name="facname" class="form-control <?php echo (!empty($facname_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $facname; ?>">
