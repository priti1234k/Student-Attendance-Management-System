...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_name)){
        $deptname_err = "Please enter vaild  name.";
    } else{
        $deptname = $input_name;
    }
    ...
    if(empty($deptname_err)){
        ...
        $sql = "INSERT INTO dept_master (DeptName , Status) VALUES (?  ,?)";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt , "ss", $param_deptname, $param_status);
...
.b{
   width:40%;
	...
	<h2 class="mt-5">Create New Department </h2>
                    
	</div>
	<div class="panel-body">
	<div class="form-group">
	<input type="text" id="deptname" name="deptname"  placeholder="Enter department " class="form-control " value="" >
